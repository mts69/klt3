  ...
  char *filename)
{
  ...
  /* Overlay features in red with relative size */
  int marker_size = (ncols < nrows) ? ncols / 100 : nrows / 100;  /* 1% of smaller dimension */
  if (marker_size < 3) marker_size = 3;  /* minimum 3x3 pixels */
  if (marker_size > 15) marker_size = 15;  /* maximum 15x15 pixels */
  ...
  for (i = 0 ; i < featurelist->nFeatures ; i++)
    if (featurelist->feature[i]->val >= 0)  {
      ...
      for (yy = y - marker_size/2 ; yy <= y + marker_size/2 ; yy++)
        for (xx = x - marker_size/2 ; xx <= x + marker_size/2 ; xx++)  
